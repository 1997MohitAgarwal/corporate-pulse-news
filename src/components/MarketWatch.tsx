...
const MarketWatch = memo(() => {
  ...
  return (
    <section id="market-watch" className="py-8 lg:py-12 bg-gradient-to-br from-slate-50 via-indigo-50/40 to-purple-50/30 relative overflow-hidden">
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-10">
          <Card className="overflow-hidden shadow-2xl hover:shadow-3xl transition-all duration-500 border-2 border-indigo-100 hover:border-indigo-400 bg-gradient-to-br from-white via-blue-50/50 to-indigo-50/30 backdrop-blur-sm animate-scale-in">
            <div className="p-4 sm:p-6 h-full flex flex-col">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-2 sm:gap-4 flex-1">
                {marketData.map((item, index) => {
                  const IconComponent = item.icon;
                  return (
                    <div key={index} className={`group p-2 sm:p-4 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 border-2 backdrop-blur-sm relative overflow-hidden flex flex-col justify-between ${
                      item.trend === 'up' 
                      ...
                      <div className="relative z-10 flex flex-col h-full justify-between min-h-[80px] sm:min-h-[120px]">
                        <div className="flex items-center justify-between mb-1 sm:mb-2">
                          <div className="flex items-center gap-1 sm:gap-2">
                            <div className={`p-0.5 sm:p-1.5 rounded-lg ${
                              item.trend === 'up' 
                                ? 'bg-emerald-100 text-emerald-600' 
                                : 'bg-red-100 text-red-600'
                            }`}>
                              <IconComponent className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5" />
                            </div>
                            <span className="font-bold text-slate-900 text-[10px] sm:text-xs tracking-wide leading-tight">{item.symbol}</span>
                          </div>
                        </div>
                        
                        <div className="text-sm sm:text-lg font-bold text-slate-900 mb-1 sm:mb-2 leading-tight">{item.value}</div>
                        
                        <div className="flex items-center justify-between">
                          <div className={`flex items-center gap-0.5 sm:gap-1 px-1 sm:px-2 py-0.5 sm:py-1 rounded-full text-[9px] sm:text-xs font-bold shadow-md transition-all duration-300 ${
                            item.trend === 'up' 
                              ? 'bg-gradient-to-r from-emerald-500 to-teal-500 text-white hover:from-emerald-600 hover:to-teal-600' 
                              : 'bg-gradient-to-r from-red-500 to-pink-500 text-white hover:from-red-600 hover:to-pink-600'
                          }`}>
                            {item.trend === 'up' ? <TrendingUp className="w-2 h-2 sm:w-3 sm:h-3" /> : <TrendingDown className="w-2 h-2 sm:w-3 sm:h-3" />}
                            <span className="leading-none">{item.change}</span>
                          </div>
                          
                          {/* Progress bar indicator */}
                          <div className="w-4 sm:w-8 bg-gray-200 rounded-full h-0.5 sm:h-1.5 overflow-hidden">
          ...
          <Card className="shadow-2xl hover:shadow-3xl transition-all duration-500 border-2 border-emerald-100 hover:border-emerald-400 bg-gradient-to-br from-white via-emerald-50/50 to-teal-50/30 backdrop-blur-sm animate-scale-in delay-200">
            <div className="p-4 sm:p-6 h-full flex flex-col">
              ...
              <div className="flex-1 overflow-hidden">
                <div className="h-full max-h-[400px] sm:max-h-[500px] overflow-y-auto space-y-4 pr-2 custom-scrollbar">
                  {marketNews.map((news, index) => (
                    <div key={index} className="p-3 sm:p-4 border-2 border-slate-100 hover:border-emerald-300 rounded-xl hover:shadow-lg transition-all duration-300 cursor-pointer bg-white/90 backdrop-blur-sm group overflow-hidden">
