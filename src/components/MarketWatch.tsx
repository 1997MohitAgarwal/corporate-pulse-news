...
const MarketWatch = () => {
  ...
  return (
    <section id="market-watch" className="py-16 lg:py-20 bg-slate-50">
      <div className="container mx-auto px-4 lg:px-6">
        <div className="text-center mb-12 lg:mb-16">
          ...
          <h2 className="text-3xl lg:text-4xl font-bold text-slate-900 mb-4">Market Watch</h2>
        ...
        <div className="grid lg:grid-cols-2 gap-6 lg:gap-8">
          {/* Market Indices - Left Side */}
          <Card className="p-4 lg:p-6 shadow-lg h-full">
            <h3 className="text-xl lg:text-2xl font-bold text-slate-900 mb-6">Live Market Indices</h3>
            <div className="grid sm:grid-cols-2 gap-4 h-full">
              {marketData.map((item, index) => (
                <div key={index} className="p-4 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-semibold text-slate-900 text-sm lg:text-base">{item.symbol}</span>
                  ...
                  <div className="text-lg lg:text-2xl font-bold text-slate-900">{item.value}</div>
                </div>
              ))}
            </div>
          </Card>

          {/* Market News - Right Side */}
          <Card className="p-4 lg:p-6 shadow-lg h-full">
            <h3 className="text-xl lg:text-2xl font-bold text-slate-900 mb-6">Market Impact News</h3>
            <div className="space-y-4 h-full">
              {marketNews.map((news, index) => (
                <div key={index} className="p-4 border border-slate-200 rounded-lg hover:border-amber-300 transition-colors cursor-pointer">
                  ...
                  <h4 className="font-semibold text-slate-900 text-sm lg:text-base leading-relaxed">
                    {news.title}
                  </h4>
                </div>
              ))}
